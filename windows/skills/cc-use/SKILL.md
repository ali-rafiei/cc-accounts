...
The same lines work from Git Bash and PowerShell; use `py -3` if `python` is missing or
opens the Microsoft Store, and a custom `CLAUDE_PROFILES` directory in place of
`~/.claude-profiles`. The argument is a profile directory name under `~/.claude-profiles/`,
or `default`. No argument means status. If the script is missing, point the user at
`install.ps1` in the `windows` folder of this plugin's repository.
...
- **The default slot holds a different account than recorded**: something wrote the login
  behind cc-use's back. Do not force it; tell the user and stop. When the message names files
